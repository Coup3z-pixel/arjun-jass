...
base_data = {
    ...
    "Social Context": {
        ...
        "Utility": [18409.320, 18409.070, 18409.150, 18409.050, 903.261, 902.650],
...
prompt_data = {
    ...
    "Social Context": {
        ...
        "Utility": [18653.930, 18653.750, 18641.590, 18653.790, 452.224, 452.276],
...
sft_data = {
    ...
    "Social Context": {
        ...
        "Utility": [451.789, 452.711, 151.500, 151.500, 903.089, 151.500],
...
def plot_comparison_bars(game_name: str, save_fig: bool = True):
    """
    Create grouped bar charts comparing Base, Prompt Injection, and SFT for each metric.
    ...
    fig.suptitle(f'{game_name} - Base vs Prompt Injection vs SFT Comparison', 
    ...
    for idx, metric in enumerate(metrics):
        ...
        bars1 = ax.bar(x - bar_width, base_values, bar_width, 
                      label='Base', color='#1f77b4', alpha=0.8, edgecolor='black')
        bars2 = ax.bar(x, prompt_values, bar_width, 
                      label='Prompt Injection', color='#ff7f0e', alpha=0.8, edgecolor='black')
        bars3 = ax.bar(x + bar_width, sft_values, bar_width,
                      label='SFT', color='#2ca02c', alpha=0.8, edgecolor='black')
        ...
        add_labels(bars1, base_values)
        add_labels(bars2, prompt_values)
        add_labels(bars3, sft_values)
    ...
    if save_fig:
        filename = f"{game_name.replace(' ', '_').replace('\'', '').lower()}_comparison.png"
...
def create_all_comparisons():
    """Create comparison charts for all games."""
    for game_name in base_data.keys():
        print(f"\nCreating comparison chart for: {game_name}")
        plot_comparison_bars(game_name)

def create_key_metrics_comparison():
    """Create a focused comparison of key altruism metrics across all conditions."""
    key_metrics = [
    ...
    fig.suptitle('Key Altruism Metrics: Base vs Prompt Injection vs SFT', 
                 fontsize=18, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('key_altruism_metrics_comparison.png', dpi=300, bbox_inches='tight')
    print("Saved: key_altruism_metrics_comparison.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    """
    # Create individual game comparisons
    plot_comparison_bars("Prisoner's Dilemma")
    plot_comparison_bars("Dictator Game")
    plot_comparison_bars("Non-Atomic Congestion")
    plot_comparison_bars("Social Context")
    
    # Create key metrics summary
    print("\nCreating key metrics comparison...")
